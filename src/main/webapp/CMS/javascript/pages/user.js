...
function ajax_self(currentPage, count, _conditions) {
    $.ajax({
        url: config.hostAddress + '/cms/user/list?pageNumKey=' + currentPage + '&pageSizeKey=' + count + _conditions,
        type: 'get',
        dataType: 'json',
        scriptCharset: 'utf-8',
        async: false, // 同步执行
...
function data_rendering(data) {
    $('#user_list').empty();//清空上一次的分页查询结果
    for (var i = 0; i < data.result.list.length; i++) {
        var dataD = data.result.list[i];
        var id = dataD.id;
        var uuid = dataD.uuid;
        var name = dataD.name;
        var mobile = dataD.mobile;
        var avatar = dataD.avatar;
        var user_password = dataD.user_password;
        var session_key = dataD.session_key;
