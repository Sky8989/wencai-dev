...
public class UserLabelServiceImpl implements UserLabelService {
	...
	public APIResponse getUserContactLabels(APIRequest apiRequest) {
		...
		if (user_contact_id <= 0)
			return APIUtil.paramErrorResponse("参数无效");
	...
	private JSONObject getUserContactLabelsByUCID(int user_contact_id) {
		...
		if (systemLabels != null) {
			List<String> system_selected_ids = label == null ? new ArrayList<>()
					: Arrays.asList(label.getSystem_label_ids().split(CUT_CHAT));
		...
		// 自定义标签
		List<LabelDTO> customLabels = label == null ? new ArrayList<>()
				: new Gson().fromJson(label.getCustom_labels(), new TypeToken<List<LabelDTO>>() {
				}.getType());
